...
export default function Register(props) 
{
    ...
    function handleChange(event)
    {
        ...
        document.getElementById("alert_register").innerText = "";
    }
    ...
    async function handleForm(e) 
    {
        ...
        createUserWithEmailAndPassword(auth, email, password).then((userCredential) => 
        {
            ...
            {
                await addDoc(collection(db, "users"), { username: username, email: email, uid: userCredential.user.uid ,/*password: hashedPassword,*/ lists: [], last: new Date()}).then(() => 
                {
                    navigate('/login');
                });
            };
        ...
        {
            document.getElementById("alert_register").innerText = error.code.split("/")[1];
        });
    }
                    ...
                    <div className='App'>
                        <header className='App-header'>
                            <form id="form_register" name="form_register" onSubmit={handleForm}>
                                ...
                                <input id="form-username"  name="username"  ref={usernameRef} onChange={handleChange} type='text' placeholder-slug="Username" placeholder='Username' required autoComplete={String(Math.random()).slice(2)} onClick={handleClick} onBlur={handleBlur} />  
                                <label htmlFor="form-email" className="opacity0" id="form-email-label">Email:</label>
                                <input id="form-email"  name="email"  ref={emailRef} type='email' onChange={handleChange} placeholder-slug="Email" placeholder='Email' required autoComplete={String(Math.random()).slice(2)} onClick={handleClick} onBlur={handleBlur} />
